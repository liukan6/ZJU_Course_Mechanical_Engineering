...
for k=tdown/dt+1:tup/dt+1
    ip(k-tdown/dt) = -out.Yt(k);%加速度滤波得到的加速度
    pp(k-tdown/dt) = out.x1(k);%滤波之后的角度
    vv(k-tdown/dt) = out.x2(k);%滤波之后的角速度
    aa(k-tdown/dt) = out.x3(k);%滤波之后的角加速度
end
% simulink模型的最小二乘法公式
phi_M = [aa' vv' pp'];
theta = inv(phi_M'*phi_M)*phi_M'*ip';  %三个参数的辨识结果
